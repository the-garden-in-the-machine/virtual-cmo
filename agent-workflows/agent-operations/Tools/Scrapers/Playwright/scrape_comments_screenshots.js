...
const visitedCodesPath = path.join(__dirname, '../../../Execution_Logs/Instagram_Data/visited_codes.json');

const getArgs = () => {
  const args = {
    username: 'fuserstudio',
    visited: path.join(__dirname, '../../../Execution_Logs/Instagram_Data/visited_codes.json')
  };
...
(async () => {
  ...
  for (let i = 0; i < codes.length; i++) {
    ...
    try {
      ...
      // Save a screenshot in the project Execution_Logs folder
      const screenshotName = `post_${code}.png`;
      const screenshotDir = path.join(__dirname, '../../../Execution_Logs/Instagram_Screenshots');
      ...
      detailedResults.push({
        ...
        screenshot: `Agent_Operations/Execution_Logs/Instagram_Screenshots/${screenshotName}`
      });
  ...
  const outPath = path.join(__dirname, `../../../Execution_Logs/Instagram_Data/scraped_details_${targetUsername}.json`);
